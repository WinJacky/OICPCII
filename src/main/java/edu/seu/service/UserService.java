...
public class UserService {
    ...
    public String register(User user, String oldEmail, String codeCaptcha, String emailCaptcha,
                           String oldCodeCaptcha, String oldEmailCaptcha) throws OICPCIIExceptions {
    ...
    public String login(String nameEmail, String password, String codeCaptcha, String oldCodeCaptcha) throws OICPCIIExceptions {
        ...
        if (old == null) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "该用户名/邮箱尚未注册！");
        }
        if (!StringUtils.equals(old.getPassword(), password)) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "密码错误！");
        }
        //设置当前用户
        setCurrentUser(old);
    ...
    private void checkBeforeRegister(User user, String oldEmail, String codeCaptcha, String emailCaptcha,
        ...
        if (StringUtils.isBlank(user.getName())) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "用户名不能为空！");
        }
        if (StringUtils.isBlank(user.getPassword())) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "密码不能为空！");
        }
        if (StringUtils.isBlank(user.getEmail()) || !EmailService.isMail(user.getEmail())) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "邮箱格式错误！");
        }
        if (!StringUtils.equals(codeCaptcha, oldCodeCaptcha)) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "验证码错误！");
        }
        if (!StringUtils.equals(user.getEmail(), oldEmail)) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "邮箱已更换，需要重新发送验证码！");
        }
        if (!StringUtils.equals(emailCaptcha, oldEmailCaptcha)) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "邮箱验证码错误！");
        }
        User old = userDao.selectByName(user.getName());
        if (old != null) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "该用户名已被注册！");
        }
        old = userDao.selectByEmail(user.getEmail());
        if (old != null) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "该邮箱已被注册！");
        }

    }

    private void checkBeforeLogin(String nameEmail, String password, String codeCaptcha, String oldCodeCaptcha) throws OICPCIIExceptions {

        if (StringUtils.isBlank(nameEmail)) {
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "用户名/邮箱不能为空！");
        }
    ...
    public void checkBeforeEmailCaptcha(String email, String codeCaptcha, String oldCodeCaptcha) throws OICPCIIExceptions {
    ...
    public void modifyPassword(String email, String newPassword, String confirmPassword, String codeCaptcha, String oldCodeCaptcha) throws OICPCIIExceptions {
    ...
    private void checkBeforeModifyPassword(String email,String newPassword, String confirmPassword,
                                           String codeCaptcha, String oldCodeCaptcha) throws OICPCIIExceptions {

        if(StringUtils.isEmpty(email)){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "邮箱不能为空！");
        }
        if(!StringUtils.equals(newPassword, confirmPassword)){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "输入密码不一致！");
        }
        if(!StringUtils.equals(codeCaptcha, oldCodeCaptcha)){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "验证码错误");
        }
    ...
    // 验证找回密码页
    public void checkBeforeFindPassword(String codeCaptcha, String oldCodeCaptcha, String emailCaptcha, String oldEmailCaptcha) throws OICPCIIExceptions {
        if(!StringUtils.equals(codeCaptcha, oldCodeCaptcha)){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "验证码错误!");
        }
        if(!StringUtils.equals(emailCaptcha, oldEmailCaptcha)){
    ...
    public void checkEmail(String email) throws OICPCIIExceptions {
        User user = userDao.selectByEmail(email);
        if(user == null){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "该邮箱尚未注册！");
        }
    }

    public void updatePassword(String oldPassword, String newPassword, String codeCaptcha,
                               String oldCodeCaptcha, String ticket) throws OICPCIIExceptions {
        User user = userDao.selectByName(currentUser.getName());
        if(!StringUtils.equals(user.getPassword(), oldPassword)){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "旧密码输入有误");
        }
        if(!StringUtils.equals(codeCaptcha, oldCodeCaptcha)){
            throw new OICPCIIExceptions(CodeEnum.USER_ERROR, "验证码错误!");
        }
